...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void addToVisor(String value) {
    setState(() {
      ...
      if (visor.isNotEmpty &&
          operacoes.contains(visor.last) &&
          operacoes.contains(value)) {
        return;
      }
  ...
  void calculate() {
    setState(() {
      ...
      var (expressao, operacoes, numeros, multDiv, somaSub) =
          _listasAuxiliares();

      if (operacoes.contains(visor.last) || visor.last == '.') {
        return; // Formato inválido
      }

      // invalida a operação se houver qualquer divisão por zero
      if (expressao.contains('÷')) {
        for (int i = 0; i < visor.length; i++) {
          if (visor[i] == '÷' && visor[(i + 1).toInt()] == '0') {
            if ((visor.length - 1) > (i + 1)) {
              if (visor[(i + 2).toInt()] == '-' ||
                  visor[(i + 2).toInt()] == '÷' ||
                  visor[(i + 2).toInt()] == '+' ||
                  visor[(i + 2).toInt()] == 'x') {
        ...
        for (int i = 0; i < multDiv.length; i++) {
          ...
          for (int j = 0; j < visor.length; j++) {
            ...
            if (operador == visor[j]) {
              ...
              if (operador == 'x') {
                tempResultado =
                    (double.parse(numAnterior) * double.parse(numPosterior));
              } else if (operador == '÷') {
                tempResultado =
                    (double.parse(numAnterior) / double.parse(numPosterior));
              }
              ...
              while (fimDaExpressao < visor.length &&
                  !operacoes.contains(visor[fimDaExpressao])) {
                fimDaExpressao++;
              }
  ...
  void _exibirResultado(double result) {
    ...
    if (result.isNegative) {
      String finalResult =
          (result % 1 == 0)
              ? result.abs().toInt().toString()
              : result.abs().toString();
      visor.add('-');
      visor.add(finalResult);
      return;
    } else if (result % 1 == 0) {
      visor.add(result.toInt().toString());
      return;
    }
